<?php
/**
 * JOURNAL アーカイブ（archive-journal）
 *
 * mockups/journal.html を正として移植。記事は news-item グリッドで一覧表示。
 * JOURNAL は内部記事（self の single-journal へ get_permalink リンク・別タブにしない）。
 * 並びは投稿日 DESC（メインクエリ既定）。
 *
 * @package bankofart
 */
...
<main id="main" class="archive-journal">

	<section class="page-hero">
		<h1 class="page-hero-title rv">JOURNAL</h1>
		<p class="page-hero-ja rv d1">読み物</p>
		<div class="page-statement rv d2">
			<p><?php echo esc_html__( 'アートと経営をめぐるコラム・インタビューをお届けします。', 'bankofart' ); ?></p>
		</div>
	</section>
	...
	<section class="news-section">
		<?php if ( have_posts() ) : ?>
			<div class="news-list" id="journalList">
				<?php
				while ( have_posts() ) :
					the_post();
					get_template_part(
